...
const processCaseRow = async (row) => {
  try {
    ...
    const day = parseValue(row['Day'], parseInt);
    const month = parseValue(row['Month'], parseInt);
    const year = parseValue(row['Year'], parseInt);
    const street_number = row['Street Number'];
    const street_name = row['Street Name'];
    const city = row['City'];
    const county = row['County'];
    const zip_code = row['Zip Code'];
    const latitude = parseValue(row['Latitude'], parseFloat);
    const longitude = parseValue(row['Longitude'], parseFloat);
    const state_id = parseValue(row['State Code'], parseInt);
    const region_id = parseValue(row['Region'], parseInt, 1);
    const civic_designation_id = parseValue(row['Urban/Suburban/Rural'], parseInt, 1);
    const metropolitan = convertToBoolean(row['Metro/Micro Statistical Area Type']);
    const location_id = parseValue(row['Location'], parseInt, 1);
    const location_specifics = row['Location Specified'];
    const insider = convertToBoolean(row['Insider or Outsider']);
    const access_type_id = parseValue(row['Access Required'], parseInt, 1);

    let access_method_id = null;
    if (row['Accessed Space'].toUpperCase() !== 'NA') {
      access_method_id = parseValue(row['Accessed Space'], parseInt, 1);
    }
    
    const victim_location_id = parseValue(row['Victims Inside / Outside'], parseInt, 1);
    ...
    const armed_bystander_id = parseValue(row['Specify Armed Person'], parseInt, 1);
    const number_killed = parseValue(row['Number Killed'], parseInt);
    const number_injured = parseValue(row['Number Injured'], parseInt);
    ...
    const values = [
      ...
      latitude, longitude, state_id, region_id, civic_designation_id, metropolitan, location_id, location_specifics, insider, access_type_id, access_method_id, victim_location_id, workplace_shooting, multiple_locations, other_location_specified, armed_person_on_scene, armed_bystander_id, number_killed, number_injured, family_member_victim, romantic_partner_victim, kidnapping_hostage_situation
    ];
